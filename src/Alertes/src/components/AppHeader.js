...
import { allReminder } from '../actions/ui/reminder';
...
function AppHeader({ month, newReminder, allReminder }) {
  ...
  useEffect(() => {
    Axios.post(env.serverPath+':'+env.servPort+'/getalerts', { "idUser": idUser }).then((result) => {
    ...
    <div className="flex px-8 py-4 flex-row gap-4 flex-nowrap shadow-lg items-center">
      <div className="w-64">
        ...
        >
          ...
          <span className="hidden lg:inline">New remider</span>
        </button>
      </div>
...
function mapDispatchToProps(dispatch) {
  return bindActionCreators({ newReminder, allReminder }, dispatch);
}
