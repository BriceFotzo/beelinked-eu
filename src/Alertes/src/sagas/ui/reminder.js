...
export function* allReminder(action) {
  const initialDate = action.payload;
  let initialDateTime = yield apply(DateTime, 'local');
  ...
  yield call(openReminder, reminder1);
}
...
export function* watchAllReminder() {
  yield takeEvery(reminderUIActions.ALL_REMINDER, allReminder);
}
