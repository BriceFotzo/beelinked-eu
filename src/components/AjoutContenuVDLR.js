...
class Form extends React.Component {
    ...
    handleContent = (idSection,idEdition,content) => {
        ...
        this.setState((prevState) => ({
            contentList: [...prevState.contentList,  { titreSection:document.getElementById(idSection).value,idEditor:idEdition.split('_')[1],contentEditor:content,idVDLR:this.props.idVDLR}],
        }));
...
this.setState((prevState) => ({
    ...
    handleSubmit = (e) => {
        ...
        this.state.newList.map((item,key)=>{
            // console.log(i)
            axios.post('http://localhost:3000/ajoutVDLRContenu/', item).then(
              res => {
                console.log(res)
              }).catch((err) => {
        ...
        for (var i = 0; i < this.state.editionList.length; i++) {
            if (this.state.editionList[i].titreVDLR === '' || this.state.editionList[i].chapitre === '') {
                    ...
                    <MDBCol></MDBCol>
                        ...
                        <MDBCol className="col-sm-10" >
                            <MDBCard className="card" style={{backgroundColor:'transparent'}}>
                                <MDBCardTitle style={{color:'white'}} className="card-header text-center">Gestion du sujet: <span className="titreFormation">{this.props.titreVDLR}</span> </MDBCardTitle>
